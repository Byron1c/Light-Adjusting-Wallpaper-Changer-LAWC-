...
//////////////////////////////////////////////////////////////////////////////////////////////////////
//  FROM https://www.codeproject.com/Articles/29306/C-Class-for-Calculating-Sunrise-and-Sunset-Times
//
...
internal sealed class SunTimes
{
                                            ...
                                            ref bool isSunrise, ref bool isSunset)
    {
        lock (mLock)    // lock for thread safety
        {
            ...
            for (int k = 0; k < 24; k++)
            {
                ...
                mVHzArr[2] = TestHour(k, t0, lat);//TestHour(k, zone, t0, lat);
        ...
        )
    {
        ...
        if (vLatDirection == LatitudeCoords.Direction.South && lat > 0) lat *= -1; // force the south values to be negative
        if (vLongDirection == LongitudeCoords.Direction.West && lon > 0) lon *= -1; // force the West values to be negative
    ...
    /// Convert latitude and longitude to degrees
    /// </summary>
    internal static void ConvertDecimalToDegrees(double vLat, double vLong,
        ref int vLatHours, ref int vLatMins, ref int vLatSecs,
        ref int vLongHours, ref int vLongMins, ref int vLongSecs
        )
    {
        ...
        //if (vLatDirection == LatitudeCoords.Direction.South && lat > 0) lat = lat * -1; // force the south values to be negative
        //if (vLongDirection == LongitudeCoords.Direction.West && lon > 0) lon = lon * -1; // force the West values to be negative
